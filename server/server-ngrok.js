...
let PUBLIC_URL = null; // Will be set by ngrok
...
app.post('/api/generate-cover', upload.single('audio'), async (req, res) => {
    ...
    try {
        ...
        // Construct public URL for the uploaded file using ngrok URL
...
async function startServer() {
    ...
    try {
        ...
        // Try to connect with or without auth token
        ...
        if (process.env.NGROK_AUTH_TOKEN && process.env.NGROK_AUTH_TOKEN.trim() !== '') {
            ...
            console.log('   Using ngrok auth token from .env');
        } else {
            console.log('   No auth token found, using free tier (may have limitations)');
        }

        PUBLIC_URL = await ngrok.connect(ngrokConfig);
        ...
        console.log('\n   Possible solutions:');
        console.log('   1. Get ngrok auth token (recommended):');
        console.log('      - Sign up at https://ngrok.com/');
        console.log('      - Get your auth token from dashboard');
        console.log('      - Add NGROK_AUTH_TOKEN=your_token to .env file');
        console.log('   2. Use alternative tunneling service (localtunnel, cloudflared)');
        console.log('   3. Deploy to a cloud platform (Heroku, Railway, Vercel)\n');
    }
}
